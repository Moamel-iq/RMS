"""
التكاليف الإضافية — a workspace over `ACCOUNT` supplier-invoice lines.

**There is no additional-cost document.** An additional cost is a
`SupplierInvoiceLine` whose `line_type` is `ACCOUNT`: a charge that never
entered stock — transport, delivery, handling, a repair, a subscription — and
whose accounting is `Dr the validated direct account / Cr Supplier Payable`.

That is the Release 1 model, and this module is a **read and navigation layer**
over it rather than a second aggregate. The reasons are worth stating, because
"additional costs" sounds like something that should have its own table:

* **It would post twice.** A cost that is both an invoice line and an
  independent document has two paths to the ledger, and the supplier is billed
  once. Choosing one authoritative owner is the whole design, and the owner is
  the invoice.
* **It would need a second source identity, a second journal and a second
  payable.** Each of those is a place for the supplier balance to disagree with
  itself.
* **Correction would fork.** A posted invoice is corrected by reversal and
  replacement. An independent cost document would need its own reversal, and the
  two would eventually be used against the same charge.

So: no create/post/reverse lives here. Posting and reversal belong to the
invoice, and this workspace links to it.

## Landed cost is deferred, not forgotten

Capitalising freight into inventory value is a real practice and is **not**
implemented, because no approved capitalization policy exists — no allocation
basis, no revaluation shape, no decision about what happens to stock already
issued. An `ACCOUNT` line therefore charges an expense or asset account
directly and changes no moving average.
"""
