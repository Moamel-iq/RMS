...
**There is no credit-term table.** A supplier's credit terms are one integer on
the supplier, and every document that cares about them takes a **snapshot** at
creation: `SupplierInvoice.payment_terms_days` beside its own `due_date`, and
the same pair on `PurchaseOrder`.

That is the Release 1 model, and the snapshot is the part that matters. A term
table with codes and names would be a nicer master-data screen and would change
nothing about correctness, because correctness here is entirely about the
snapshot: an invoice raised at 14 days stays at 14 days forever, whatever the
supplier's default becomes afterwards. Reading terms live from the supplier at
display time would silently restate the due date of every historical invoice
the moment somebody renegotiated, including invoices already posted, already
paid, and already chased.

So this module reads. It shows what each supplier's default is, what that means
in days, and how their open invoices are sitting against their own snapshots.
Updating a supplier's default goes through the supplier service; nothing here
writes to an invoice.
"""
