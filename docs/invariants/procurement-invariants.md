...
| 41 | A credit note reduces the payable or stands as unallocated credit; it moves no stock | Both are allocation states against the payable; `test_an_allocation_reduces_the_invoice_outstanding`, `test_it_never_moves_stock` | 2.14 |
| 41a | A note settles its return through explicit line allocations — partially, and a line may take several notes — bounded by the line's returned quantity and posted book value | `SupplierCreditReturnAllocation` under a return-line lock; `test_two_partial_notes_settle_one_return`, `test_crediting_more_than_the_line_returned_is_refused` and the two-notes race | 2.14 |
| 41a′ | A partial slice settles the quantized proportional share of the remaining claim; the final slice takes the exact remainder, so no rounding residual strands in the clearing account | `test_a_proportional_slice_and_an_exact_remainder_on_an_odd_value`; `verify_supplier_credit_notes`' residual check | 2.14 |
| 41b | The return variance is recognised only at the credit note, at agreed-versus-book, either direction, absent when equal | ADR-022 §2 as amended; `test_a_dearer_credit_recognises_the_gain`, `test_a_cheaper_credit_recognises_the_loss`, `test_an_agreeing_credit_posts_two_lines_and_no_variance` | 2.14 |
