...
| PRC-094 | Payment terms attach to a settlement cycle opened by the first invoice, not to each invoice independently | `SupplierPaymentCycle` + `cycle_for_invoice` | `apps/procurement/tests/test_supplier_payment_cycles.py::TestTheCycleWindow::test_the_first_invoice_opens_a_cycle_due_the_terms_later` | Supplier settlement | | Done |
| PRC-095 | A later invoice joins the running cycle and shares its due date, with only the remaining days — never a fresh term | `cycle_for_invoice` + `days_remaining` | `apps/procurement/tests/test_supplier_payment_cycles.py::TestTheCycleWindow::test_a_later_invoice_joins_it_and_shares_the_due_date` | Supplier settlement | | Done |
| PRC-096 | A cycle stops collecting at its due date; the next invoice opens a new one and the old window stays owed | `SupplierPaymentCycleStatus.DUE` + `procurement_cycle_one_collecting_per_supplier` | `apps/procurement/tests/test_supplier_payment_cycles.py::TestTheCycleWindow::test_an_invoice_after_the_due_date_opens_a_new_cycle` | Supplier settlement | | Done |
| PRC-097 | The terms and the settlement floor are snapshotted when a cycle opens, so renegotiating later restates no history | `SupplierPaymentCycle.payment_terms_days` / `.minimum_settlement_percent` | `apps/procurement/tests/test_supplier_payment_cycles.py::TestTheCycleWindow::test_the_terms_and_the_floor_are_snapshotted` | Supplier settlement | | Done |
| PRC-098 | A cycle closes when its invoices are paid off, and a reversed payment reopens it as DUE, never as COLLECTING | `close_cycle_if_settled` + `reopen_cycle` | `apps/procurement/tests/test_settlement_workspace.py::TestPostingASettlement::test_a_settled_cycle_closes_and_the_next_invoice_opens_a_new_one` | Supplier settlement | | Done |
| PRC-099 | تسديد الموردين shows the cycle dates, the unsettled total, the agreed share and the required amount, with open invoices oldest first | `workspace_for` + `SupplierSettlementView` | `apps/procurement/tests/test_settlement_workspace.py::TestTheScreen::test_it_renders_the_cycle_the_balance_and_the_plans` | Supplier settlement | | Done |
| PRC-100 | Where the target matches no run of whole invoices the screen offers التسديد الأقل, التسديد الأعلى and an exact match, each with its count, invoices, total and difference | `_under` / `_over` / `_exact` + `PlanRow` | `apps/procurement/tests/test_settlement_workspace.py::TestTheWorkspace::test_the_three_plans_bracket_the_target` | Supplier settlement | | Done |
| PRC-101 | Settlement is FIFO, oldest invoice first, whichever plan is chosen and whatever order the caller passes | `open_owings` + `draft_settlement` | `apps/procurement/tests/test_settlement_workspace.py::TestDraftingASettlement::test_the_allocations_are_written_oldest_first` | Supplier settlement | | Done |
| PRC-102 | Confirming a plan drafts an ordinary supplier payment whose amount is the sum of its allocations; posting stays a separate permission | `draft_settlement` + `post_supplier_payment` | `apps/procurement/tests/test_settlement_workspace.py::TestDraftingASettlement::test_the_amount_is_the_sum_of_its_allocations` | Supplier settlement | | Done |
| PRC-103 | A settlement cannot pay more than an invoice owes; an advance remains only where a payment is keyed by hand | `add_payment_allocation` over-allocation guard | `apps/procurement/tests/test_settlement_workspace.py::TestDraftingASettlement::test_it_cannot_pay_more_than_an_invoice_owes` | Supplier settlement | | Done |
| PRC-104 | A balance that moved since the screen was read is refused with the new figures rather than settled | `shown_open_total` drift check in `SupplierSettlementView.post` | `apps/procurement/tests/test_settlement_workspace.py::TestTheScreen::test_a_balance_that_moved_is_refused_rather_than_paid` | Supplier settlement | | Done |
| PRC-105 | Partial settlement happens only where the exact-match plan is chosen, and the remainder stays owed on that invoice | `_exact` + `PlannedAllocation.is_partial` | `apps/procurement/tests/test_settlement_workspace.py::TestDraftingASettlement::test_the_partial_plan_leaves_the_rest_of_that_invoice_owed` | Supplier settlement | | Done |
| PRC-106 | Unsettled cycles are reported soonest due first with days remaining, outstanding and the required amount | `reports.payment_cycles` + `PaymentCycleReportView` | `apps/procurement/tests/test_procurement_reports.py` | Supplier settlement | | Partial |
| INS-001 | A finding has one stable identity for its lifetime, unique per organization, detector and fingerprint | `Insight` + `insights_identity_unique` | `apps/insights/tests/test_insight_kernel.py::TestIdentityAndObservations::test_first_detection_opens_a_case_with_one_observation_and_one_event` | Insights stage 1 | | Done |
| INS-002 | Re-detection appends an observation and never rewrites severity, evidence or narrative | `InsightObservation` + `_persist_candidate` | `apps/insights/tests/test_insight_kernel.py::TestIdentityAndObservations::test_re_detection_appends_and_never_rewrites` | Insights stage 1 | | Done |
| INS-003 | One run may observe one case at most once, enforced by the database | `insights_observation_unique_per_run` | `apps/insights/tests/test_insight_kernel.py::TestIdentityAndObservations::test_the_same_run_cannot_observe_a_case_twice` | Insights stage 1 | | Done |
| INS-004 | Observations and lifecycle events cannot be updated or deleted | `insights/0002_insight_append_only` | `apps/insights/tests/test_insight_kernel.py::TestImmutability::test_an_observation_cannot_be_updated` · `apps/insights/tests/test_insight_kernel.py::TestImmutability::test_a_lifecycle_event_cannot_be_deleted` | Insights stage 1 | | Done |
| INS-005 | A run may record only its completion; its window and scope are immutable | `insights_run_stamp_only` allowlist trigger | `apps/insights/tests/test_insight_kernel.py::TestImmutability::test_a_run_may_only_record_its_completion` | Insights stage 1 | | Done |
| INS-006 | Current status is derived from the latest event and is never stored on the identity | `selectors.visible_insights` annotations | `apps/insights/tests/test_insight_screens.py::TestDashboardAccess::test_the_counts_come_from_the_same_filtered_queryset` | Insights stage 1 | | Done |
| INS-007 | Auto-resolution requires a STATEFUL detector, a SUCCEEDED run, COMPLETE coverage and an explicitly evaluated fingerprint | `services._auto_resolve` | `apps/insights/tests/test_insight_kernel.py::TestAutoResolution::test_a_complete_clean_run_resolves_a_case_it_evaluated` | Insights stage 1 | | Done |
| INS-008 | A PARTIAL run resolves nothing, and a case it never evaluated is never resolved | `services._auto_resolve` | `apps/insights/tests/test_insight_kernel.py::TestAutoResolution::test_a_partial_run_resolves_nothing` · `apps/insights/tests/test_insight_kernel.py::TestAutoResolution::test_a_case_the_run_never_evaluated_is_not_resolved` | Insights stage 1 | | Done |
| INS-009 | A dismissal requires a reason and suppresses automatic reopening until a human reopens it | `dismiss_insight` + `insights_event_decision_needs_reason` | `apps/insights/tests/test_insight_kernel.py::TestAutoResolution::test_a_dismissed_case_is_not_resolved_and_not_reopened_by_a_run` · `apps/insights/tests/test_insight_kernel.py::TestLifecycleActions::test_dismissal_requires_a_reason` | Insights stage 1 | | Done |
| INS-010 | A resolved condition that recurs reopens the same case rather than opening a second one | `services._persist_candidate` | `apps/insights/tests/test_insight_kernel.py::TestAutoResolution::test_a_resolved_case_reopens_on_recurrence` | Insights stage 1 | | Done |
| INS-011 | One detector failure is isolated, recorded, and never erases a sibling's outcome or leaks a traceback | `services._run_one_detector` | `apps/insights/tests/test_insight_kernel.py::TestOutcomes::test_a_failing_detector_is_isolated_and_recorded` · `apps/insights/tests/test_insight_kernel.py::TestOutcomes::test_a_failure_never_leaks_its_traceback` | Insights stage 1 | | Done |
| INS-012 | Terminal outcome and coverage are recorded independently of each other | `InsightDetectorOutcome` | `apps/insights/tests/test_insight_kernel.py::TestOutcomes::test_a_skipped_prerequisite_is_not_a_failure` · `apps/insights/tests/test_insight_kernel.py::TestOutcomes::test_an_empty_successful_run_is_recorded_as_success` | Insights stage 1 | | Done |
| INS-013 | Evidence rejects a binary float at any depth and stores every Decimal as an exact string | `services.assert_no_floats` | `apps/insights/tests/test_insight_kernel.py::TestEvidenceHygiene::test_a_float_anywhere_in_the_evidence_is_refused` · `apps/insights/tests/test_issue_coverage_detector.py::TestEvidenceAndWording::test_every_decimal_in_the_evidence_is_an_exact_string` | Insights stage 1 | | Done |
| INS-014 | inventory-issue-coverage-gap emits a finding when actual divided by theoretical consumption is strictly below the configured share | `detectors.inventory_issue_coverage.detect` | `apps/insights/tests/test_issue_coverage_detector.py::TestTheRatio::test_zero_recorded_consumption_is_a_high_finding` · `apps/insights/tests/test_issue_coverage_detector.py::TestTheRatio::test_a_small_but_non_zero_share_is_medium` | Insights stage 1 | | Done |
| INS-015 | Equality with the threshold is not a finding; the boundary is strictly less-than on exact Decimals | `detect` ratio comparison | `apps/insights/tests/test_issue_coverage_detector.py::TestTheRatio::test_exactly_at_the_threshold_is_not_a_finding` · `apps/insights/tests/test_issue_coverage_detector.py::TestTheRatio::test_just_below_the_threshold_is_a_finding` | Insights stage 1 | | Done |
| INS-016 | Severity is HIGH when recorded consumption is exactly zero and MEDIUM when it is non-zero below the threshold | `detect` severity rule | `apps/insights/tests/test_issue_coverage_detector.py::TestTheRatio::test_zero_recorded_consumption_is_a_high_finding` | Insights stage 1 | | Done |
| INS-017 | An item is excluded, never guessed at, when it has no posted receipt, no theoretical consumption, a unit mismatch, or a broken stock identity | `detect` eligibility gates | `apps/insights/tests/test_issue_coverage_detector.py::TestWhenItRefusesToSpeak::test_no_theoretical_consumption_means_no_denominator` · `apps/insights/tests/test_issue_coverage_detector.py::TestWhenItRefusesToSpeak::test_a_unit_mismatch_is_excluded_not_compared` · `apps/insights/tests/test_issue_coverage_detector.py::TestWhenItRefusesToSpeak::test_a_broken_stock_identity_suppresses_the_item` | Insights stage 1 | | Done |
| INS-018 | Excluded items degrade coverage to PARTIAL, and an empty evaluated set is INSUFFICIENT rather than COMPLETE | `detect._coverage` | `apps/insights/tests/test_issue_coverage_detector.py::TestWhenItRefusesToSpeak::test_partial_coverage_when_some_items_are_excluded` | Insights stage 1 | | Done |
| INS-019 | The fingerprint carries only stable technical identity, never the period, severity, measured ratio or a translated label | `detect._fingerprint` | `apps/insights/tests/test_issue_coverage_detector.py::TestDeterminism::test_the_fingerprint_carries_no_rolling_value` | Insights stage 1 | | Done |
| INS-020 | The same inputs produce the same candidates, evidence and ordering | `detect` + `sort_key` | `apps/insights/tests/test_issue_coverage_detector.py::TestDeterminism::test_the_same_inputs_produce_the_same_output` · `apps/insights/tests/test_issue_coverage_detector.py::TestDeterminism::test_worst_ratio_sorts_first` | Insights stage 1 | | Done |
| INS-021 | The narrative reports incomplete recording and never asserts waste, theft or responsibility | `detect._narrative` and `_recommendation` | `apps/insights/tests/test_issue_coverage_detector.py::TestEvidenceAndWording::test_the_narrative_reports_a_recording_gap_not_an_accusation` | Insights stage 1 | | Done |
| INS-022 | Evidence carries the period, cutoffs, formula and versions needed to reproduce the claim, with typed source references and no URLs | `detect._evidence` | `apps/insights/tests/test_issue_coverage_detector.py::TestEvidenceAndWording::test_the_evidence_carries_what_reproduces_the_claim` · `apps/insights/tests/test_issue_coverage_detector.py::TestEvidenceAndWording::test_source_references_are_typed_never_urls` | Insights stage 1 | | Done |
| INS-023 | Findings are visible only with insights.view_insight plus organization scope; another organization's finding is 404, never 403 | `selectors.visible_insights` + `resolve_insight` | `apps/insights/tests/test_insight_screens.py::TestDashboardAccess::test_another_organization_sees_nothing` · `apps/insights/tests/test_insight_screens.py::TestDashboardAccess::test_the_detail_of_another_organization_is_404_not_403` | Insights stage 1 | | Done |
| INS-024 | Reading and deciding are separate grants: manage_insight gates every lifecycle action at the POST, not only in the template | `InsightLifecycleView` | `apps/insights/tests/test_insight_screens.py::TestLifecycleOverHttp::test_a_reader_without_manage_cannot_act` · `apps/insights/tests/test_insight_screens.py::TestLifecycleOverHttp::test_the_manage_buttons_are_hidden_from_a_reader` | Insights stage 1 | | Done |
| INS-025 | Lifecycle actions are POST-only; no state change is reachable by GET | `InsightLifecycleView` | `apps/insights/tests/test_insight_screens.py::TestLifecycleOverHttp::test_acknowledge_requires_post` | Insights stage 1 | | Done |
| INS-026 | Dashboard counts and list come from one filtered selector, and the htmx fragment matches the full page | `selectors.headline_counts` + dashboard template | `apps/insights/tests/test_insight_screens.py::TestHtmxParity::test_the_fragment_carries_no_second_document` · `apps/insights/tests/test_insight_screens.py::TestHtmxParity::test_the_full_page_and_the_fragment_show_the_same_findings` | Insights stage 1 | | Done |
| INS-027 | Evidence is rendered as escaped text; the template never bypasses autoescaping | `templates/insights/detail.html` | `apps/insights/tests/test_insight_screens.py::TestDetailAndEvidence::test_evidence_is_rendered_as_text_not_markup` | Insights stage 1 | | Done |
| INS-028 | The sidebar entry is hidden from a reader without the permission | `apps/core/navigation.py` + `navigation_access` | `apps/insights/tests/test_insight_screens.py::TestNavigation::test_the_module_is_hidden_from_someone_without_the_permission` | Insights stage 1 | | Done |
