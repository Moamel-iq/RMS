...
| RCP-033 | Draft batches editable and deletable; posted immutable except reversal | `kitchen/0011` whole-row triggers + `production_batch_is_draft_only_until_task_3_5` | `apps/kitchen/tests/test_production_contracts.py::TestTheDecisionIsFrozen` · `apps/kitchen/tests/test_production_services.py::TestTheDatabaseRefusesWhatTheServiceRefuses` | 3.4 / 3.5 | The posted half needs a posted batch, which arrives at 3.5 | Partial |
